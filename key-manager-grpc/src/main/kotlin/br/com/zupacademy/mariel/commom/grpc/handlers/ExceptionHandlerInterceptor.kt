...
class ExceptionHandlerInterceptor : MethodInterceptor<CadastroNovaChavePixEndPoint, Any?> { // or BindableService

    private val LOGGER = LoggerFactory.getLogger(ExceptionHandlerInterceptor::class.java)

    override fun intercept(context: MethodInvocationContext<CadastroNovaChavePixEndPoint, Any?>): Any? {
        try {
            return context.proceed()
        } catch (e: Exception) {
            LOGGER.error("Error: ${e.javaClass} ${e.message}")
